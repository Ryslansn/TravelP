...
## Перед публікацією заповнити

У тексті є плейсхолдери. Вони оформлені як `code`, тож незаповнений одразу
видно на сторінці. Знайти всі:

```bash
grep -rn 'ЗАПОВНІТЬ\|FILL IN' .
```

| Що | Де |
|---|---|
| назва особи або компанії (володілець даних) | privacy, terms |
| поштова адреса | privacy |
| email підтримки | усі чотири |
| строк перезапису резервних копій | privacy, delete-account |
| мінімальний вік (13 або 16) | privacy, terms |
| юрисдикція | terms |

**Email підтримки** має бути окремою адресою, а не особистою поштою — він
потрапляє в лістинг стору й стає публічним.
